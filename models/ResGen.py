...
class Res2Mol(Module):
    ...
    '''
    def __init__(self, config, num_classes, num_bond_types, protein_res_feature_dim, ligand_atom_feature_dim):
        super().__init__()
        self.config = config
    ...
    def get_loss(self, pos_real, y_real, pos_fake,# query real positions,
                       ...
                       compose_feature, compose_pos, compose_vec, idx_ligand, idx_protein,  # compose (protein and context ligand) atoms 
    ...
    def sample_init(self,
        ...
        focal_resutls = self.sample_focal(compose_feature, compose_vec, compose_pos, idx_ligand, idx_protein, compose_knn_edge_index, compose_knn_edge_feature)
    ...
    def sample(self,
        ...
        focal_resutls = self.sample_focal(compose_feature, compose_vec, compose_pos, idx_ligand, idx_protein, compose_knn_edge_index, compose_knn_edge_feature, frontier_threshold=frontier_threshold)
        if focal_resutls[0]:  # has frontiers
            has_frontier, idx_frontier, p_frontier, idx_focal_in_compose, p_focal, h_compose = focal_resutls
            pos_generated, pdf_pos, idx_parent, abs_pos_mu, pos_sigma, pos_pi = self.sample_position(
                h_compose, compose_pos, idx_focal_in_compose, n_samples=n_samples_pos
    ...
    def get_tri_edges(self, edge_index_query, pos_query, idx_ligand, ligand_bond_index, ligand_bond_type):
        ...
        n_context = len(idx_ligand)
        adj_mat = (torch.zeros([n_context, n_context], dtype=torch.long) - torch.eye(n_context, dtype=torch.long)).to(device)
